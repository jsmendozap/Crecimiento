...
6. Cuando le pida la ruta para instalar miniconda ingrese: `/home/<usuario>/.local/share/r-miniconda` cambiando `<usuario>` por el nombre de su usuario en el computador.
7. Instalar el servicio de ssh con `apt install openssh` 
8. Ejecutar el comando `ssh-keygen -t rsa -b 4096`, establecer las claves u oprimir enter para dejar vacio este espacio 
9. Ejecutar el comando `ssh-copy-id usuario@ip -p 8022` cambiando usuario por el nombre de usuario del telefono (lo obtiene en termux con `whoami` y la ip por la ip del teléfono).
10. Halle la escala de la fotografía (puede ser hallada a traves de software como ImageJ)
11. Ejecutar el script `procesamiento.R` para iniciar la aplicación poniendo como argumento el valor obtenido en el punto anterior así: `./procesamiento.R valor`
